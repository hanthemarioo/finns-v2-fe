...
function normalizeOptions(value: unknown): GrowerReportSelectOption[] {
...
export function GrowerReportClient({
    filters,
    farms,
    flocks,
    coops,
    ...
    const [periodType, setPeriodType] = useState<GrowerReportPeriodType>(filters.period_type);
    ...
    const columns = useMemo(() => createGrowerReportTableColumns(), []);
    ...
    const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
        ...
        const nextPeriodType = String(formData.get("period_type") ?? "30_days") as GrowerReportPeriodType;
            ...
            >
                ...
                <label className="space-y-1 text-sm font-medium text-gray-700">
                    ...
                    <select
                        name="period_type"
                        value={periodType}
                        onChange={(event) => setPeriodType(event.target.value as GrowerReportPeriodType)}
                        className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                    >
